...
#
# Both serve on http://localhost:$PORT. --watch needs docs/requirements-dev.txt;
# the one-shot build only needs docs/requirements.txt.
#
...
PORT="${PORT:-8000}"
CONTRACT="rust-lib/chat_module.lidl"
...
echo "==> Rendering the contract…"
./docs/lidl2rst.py "$CONTRACT" -o docs/_generated

if [ -n "$WATCH" ]; then
  echo "==> Watching for changes, serving on http://localhost:$PORT  (Ctrl-C to stop)"
  # docs/_generated is rewritten on every pre-build, so watching it would loop.
  exec sphinx-autobuild docs docs/_build/html \
    --host 0.0.0.0 \
    --port "$PORT" \
    --watch "$CONTRACT" \
    --pre-build "./docs/lidl2rst.py $CONTRACT -o docs/_generated" \
    --ignore '*/_generated/*' \
    --ignore '*/_build/*' \
    --ignore '*/site/*'
fi
