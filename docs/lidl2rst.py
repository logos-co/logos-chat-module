...
def render_callable(decl, comments, objtype):
    out = [f".. lidl:{objtype}:: {signature(decl)}", ""]
...
def render(module, comments, source_name):
    sections = {
        "methods.rst": [
            render_callable(m, comments, "method") for m in module["methods"]
        ],
        "events.rst": [render_callable(e, comments, "event") for e in module["events"]],
...
def clashing_anchors(module):
    """Declarations the LIDL domain would give the same link target.

    Records are prefixed, so only a method and an event sharing a name collide.
    """
    seen = {}
    for objtype in ("methods", "events"):
        for decl in module[objtype]:
            seen.setdefault(decl["name"], []).append(objtype[:-1])
...
def main():
    ...
    if clashes:
        ...
        sys.exit(f"{args.contract}: declarations would share a link target -- {detail}")
