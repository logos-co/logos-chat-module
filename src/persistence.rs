...
//! same value passed to `chat_module_init`. Format is the pretty-printed
...
pub(crate) fn save_state(state: &AppState, path: &Path) {
    let json = match serde_json::to_string_pretty(state) {
        Ok(j) => j,
        Err(e) => {
            eprintln!("chat_module: save_state serialise failed: {e}");
            return;
        }
    };
    ...
    if let Err(e) = fs::write(&tmp, json).and_then(|_| fs::rename(&tmp, path)) {
        eprintln!("chat_module: save_state write failed: {e}");
    }
}
