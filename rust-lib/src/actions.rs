...
// ── Contract view types ──────────────────────────────────────────────────────
//
// There are none any more, deliberately. `Conversation`, `Message`, `Status`
// and `GroupMember` are generated from chat_module.lidl (see the `generated`
// module in lib.rs) and are the types these functions return, so the contract
// is checked by the compiler instead of by hand-written serde shapes that
// happened to agree with it.
//
// What that removes is a silent failure mode: the mirrors serialized through
// `serde_json::to_value(..).unwrap_or_else(|_| empty)`, so a field that drifted
// from the record produced an empty list rather than an error. There is no
// encode step left to fail.
//
// The internal shapes stay internal — `persistence::DisplayMessage` and the
// `ConversationKind` / `DeliveryStateKind` enums are the module's own state, and
// the mapping to the contract's `tstr` spellings happens here at the boundary.
...
/// The roster of the conversation `convo_id`, one contract `GroupMember` per
...
/// conversation, or a client error, yields an empty list (the client error is
/// logged).
pub(crate) fn list_group_members(convo_id: &str) -> Vec<crate::GroupMember> {
    if !with_display(|d| d.state.chats.contains_key(convo_id)) {
        return Vec::new();
    }
    match with_client(|client| client.group_members(convo_id)) {
        // `pending` is read BEFORE `member_address` takes `m` by value; the
        // field order here is load-bearing, not cosmetic.
        Ok(Ok(members)) => members
            .into_iter()
            .map(|m| crate::GroupMember {
                pending: m.pending,
...
pub(crate) fn list_conversations() -> Vec<crate::Conversation> {
    with_display(|d| {
        d.state
            .chats
            .values()
            .map(|s| crate::Conversation {
                convo_id: s.chat_id.clone(),
                nickname: s.nickname.clone(),
                // The contract spells counts and timestamps `int`, i.e. i64.
                // Both are non-negative and bounded by things that cannot
                // approach i64::MAX here — a per-conversation message count and
                // a millisecond epoch — so these casts cannot wrap in practice.
                message_count: s.messages.len() as i64,
                last_activity_ms: s.messages.last().map(|m| m.timestamp_ms).unwrap_or(0) as i64,
                // The enum is internal; `tstr` on the wire. as_str() is the
...
pub(crate) fn get_messages(convo_id: &str) -> Vec<crate::Message> {
    with_display(|d| {
        d.state
            .chats
            .get(convo_id)
...
pub(crate) fn status() -> crate::Status {
    with_display(|d| crate::Status {
